...
const ALL_CATS = [
  { id: "conj", label: "Conjunction", short: "conj", color: "#4f8ef7", emoji: "🔗" },
  { id: "prep", label: "Preposition", short: "prep", color: "#f76f4f", emoji: "📍" },
  { id: "adv",  label: "Adverb",      short: "adv",  color: "#4fcf8a", emoji: "⚡" },
  { id: "both", label: "Both",        short: "both", color: "#f7c94f", emoji: "🔀" },
];

const isPartSixOrSeven = (part) => part === "part6" || part === "part7" || part === "part6-7";
...
const getCorrectCat = (row, part) => {
  if (!row) return null;
  if (isPartSixOrSeven(part)) return normalizeCatId(row.dual_type || row.word_type);
...
function QuizPage() {
  ...
  const part = useMemo(() => params.part || location.state?.part || "", [params.part, location.state]);
  ...
  useEffect(() => {
    ...
    if (!isSupabaseConfigured || !supabase) {
      setError("Supabase ยังไม่ได้ตั้งค่า"); setLoading(false); return;
    }
    let cancelled = false;
    ...
    query
      .then(({ data, error: err }) => {
        ...
        setRows(s); setDeck(s); setLoading(false);
      })
  ...
  useEffect(() => {
    setIdx(0); setSelected(null); setRevealed(false); setScore(0); setTotal(0); setDone(false);
  }, [deck]);
  ...
  const diffColor = current?.difficulty === 1 ? "#4fcf8a" : current?.difficulty === 2 ? "#f7c94f" : "#f76f4f";
  const diffLabel = current?.difficulty === 1 ? "Beginner" : current?.difficulty === 2 ? "Intermediate" : "Advanced";

  const CHEERS_OK = ["เก่งมาก! 🎉", "ถูกต้อง! ✨", "ยอดเยี่ยม! 🌟", "แม่นมาก! 💯"];
  const CHEERS_NO = ["เกือบแล้ว! 🏋️", "ทบทวนด้วยนะ! 📖", "อย่าท้อ! 🔥"];
  const cheer = isCorrect ? CHEERS_OK[total % CHEERS_OK.length] : CHEERS_NO[total % CHEERS_NO.length];
  ...
  if (done) {
    ...
    const msg = { S:"พร้อมสอบ TOEIC! 🏆", A:"เก่งมาก! 🌟", B:"ทบทวนคำยากอีกนิด! 📚", C:"ลองใหม่ได้! 💪", D:"ทบทวนเพิ่มนะ! 📖" }[grade];
    return (
      <div style={S.root}>
        <style>{css}</style>
        <div style={S.resultCard}>
          <span style={{ ...S.pill, background: `${modeColor}1a`, color: modeColor, border: `1px solid ${modeColor}44` }}>{modeLabel}</span>
          <div style={S.rGrade}>{grade}</div>
          <div style={S.rScore}>{score} / {total}</div>
          <div style={{ fontSize: "0.9rem", color: "#7a84a8" }}>{pct}% ถูกต้อง</div>
          <div style={{ fontSize: "0.9rem", color: "#c8d5f0", textAlign: "center", lineHeight: 1.6 }}>{msg}</div>
          <div style={{ display: "flex", gap: 10, width: "100%", marginTop: 8 }}>
            <button style={{ ...S.btn, background: "#1e2a40", color: "#c8d5f0", flex: 1 }} onClick={() => navigate("/")}>← หน้าหลัก</button>
            <button style={{ ...S.btn, background: modeColor, flex: 1 }} onClick={restart}>เล่นอีกครั้ง</button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={S.layout}>
        ...
        <div style={S.topPanel}>
          ...
          <div style={S.headerBar}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ ...S.pill, background: `${modeColor}18`, color: modeColor }}>{modeLabel}</span>
            </div>
            <div style={S.scoreBox}>
              <span style={S.scoreLabel}>คะแนน</span>
              <span style={S.scoreVal}>{score}<span style={S.scoreOf}>/{total}</span></span>
          ...
          {deck.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 6 }}>
              <div style={S.progressTrack}>
                <div style={{ ...S.progressFill, width: `${(idx / deck.length) * 100}%`, background: modeColor }} />
              </div>
              <span style={{ fontSize: "0.68rem", color: "#7a84a8", whiteSpace: "nowrap" }}>{idx + 1}/{deck.length}</span>
            </div>
          )}
          ...
          {!part ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, marginTop: 24 }}>
              ...
              <span style={{ color: "#7a84a8", fontSize: "0.9rem" }}>กำลังโหลดคำศัพท์...</span>
            </div>
          ) : error ? (
            ...
            <div key={animKey} style={S.questionCard} className="card-in">
              ...
              {/* Choice buttons */}
              <div style={{ display: "grid", gridTemplateColumns: activeCats.length === 2 ? "1fr 1fr" : "1fr 1fr", gap: 8, width: "100%" }}>
                {activeCats.map((cat) => {
                  let bg = "#1e2235", border = "2px solid #2e3450", color = "#c8d0e8";
                  if (revealed && selected === cat.id) {
                    ...
                    bg = "#0f2a1e"; border = "2px solid #4fcf8a"; color = "#4fcf8a";
                  }
                  return (
                    <button key={cat.id} className="qbtn" disabled={revealed}
                      onClick={() => handleSelect(cat.id)}
                      style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:2,
                        padding:"11px 8px", borderRadius:12, border, background:bg, color,
                        cursor: revealed ? "default" : "pointer", fontFamily:"'DM Sans',sans-serif",
                        transition:"all 0.18s" }}>
                      <span style={{ fontSize:"1.15em" }}>{cat.emoji}</span>
                      <span style={{ fontWeight:600, fontSize:"0.85rem" }}>{cat.label}</span>
                      <span style={{ fontSize:"0.68rem", opacity:0.5 }}>({cat.short})</span>
                    </button>
                  );
                })}
              </div>
        ...
        <div style={S.bottomPanel}>
          {revealed && current && (
            <div style={S.feedback(isCorrect)} className="feed-in">
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:6 }}>
                <span style={{ fontFamily:"'Playfair Display',serif", fontSize:"1rem", fontWeight:700, color:"#e8d5a3" }}>
                  {isCorrect ? "✅" : "❌"} {cheer}
                </span>
                <button className="qbtn" onClick={handleNext}
              ...
              <div style={S.infoGrid}>
                <InfoBox label="🎯 Part of Speech" val={ALL_CATS.find(c => c.id === correctCat)?.label ?? correctCat} />
              ...
              {(current?.example_1 || current?.example_2) && (
                <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
                  ...
                  {current.example_1 && <div style={S.exLine} dangerouslySetInnerHTML={{ __html: current.example_1 }} />}
                  {current.example_2 && <div style={S.exLine} dangerouslySetInnerHTML={{ __html: current.example_2 }} />}
                </div>
              )}
              ...
              {current?.meaning_en && (
                <div style={{ display:"flex", flexDirection:"column", gap:3 }}>
                  <div style={S.sectionLabel}>🇬🇧 Meaning (EN)</div>
                  <div style={{ fontSize:"0.82rem", color:"#8a94b8" }}>{current.meaning_en}</div>
                </div>
              )}
            </div>
          )}
          ...
          {!revealed && deck.length > 0 && !loading && (
            <div style={S.hintBox}>
              👆 เลือกคำตอบด้านบนเพื่อดูคำอธิบาย
            </div>
          )}
        </div>
...
function InfoBox({ label, val }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
      <div style={{ fontSize:"0.62rem", fontWeight:700, color:"#7a84a8", textTransform:"uppercase", letterSpacing:"0.08em" }}>{label}</div>
      <div style={{ fontSize:"0.88rem", color:"#c8d5f0", fontWeight:500, lineHeight:1.4 }}>{val}</div>
    </div>
  );
}
...
const S = {
  ...
  backBtn: {
    background: "#1e2235", border: "1.5px solid #2e3450", color: "#7a84a8",
    borderRadius: 9, padding: "5px 12px", cursor: "pointer",
    fontFamily: "'DM Sans',sans-serif", fontSize: "1rem",
  },

  pill: { fontSize: "0.7rem", fontWeight: 700, borderRadius: 99, padding: "3px 11px", letterSpacing: "0.06em" },

  scoreBox: {
    display: "flex", alignItems: "center", gap: 5,
    background: "#1e2235", border: "1.5px solid #2e3450",
    borderRadius: 9, padding: "4px 11px",
  },
  scoreLabel: { fontSize: "0.62rem", color: "#7a84a8", textTransform: "uppercase", letterSpacing: "0.08em" },
  ...
  progressTrack: {
    flex: 1, height: 4,
    background: "#1e2235", borderRadius: 99, overflow: "hidden",
  },
  ...
  diffBadge: { fontSize: "0.63rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", borderRadius: 99, padding: "2px 9px" },
  groupBadge: { fontSize: "0.63rem", color: "#7a84a8", background: "#1e2235", border: "1px solid #2e3450", borderRadius: 99, padding: "2px 9px" },
  ...
  sectionLabel: { fontSize: "0.63rem", fontWeight: 700, color: "#7a84a8", textTransform: "uppercase", letterSpacing: "0.08em" },

  exLine: {
    fontSize: "0.82rem", color: "#b0bcd8", lineHeight: 1.65,
    borderLeft: "3px solid #2e3a58", paddingLeft: 9,
  },
  ...
  btn: {
    padding: "11px 0", width: "100%", borderRadius: 12,
  ...
  infoText: {
    marginTop: 32, fontSize: "0.9rem", color: "#7a84a8", textAlign: "center",
  },

  resultCard: {
    margin: "auto",
    background: "#161b2e", border: "1.5px solid #252c44",
  ...
  rGrade: {
    ...
    fontWeight: 900, color: "#f7c94f", lineHeight: 1,
  },
  rScore: {
